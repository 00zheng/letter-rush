...
- New, duplicate, and invalid submissions show brief floating feedback plus
  debounced `aria-live` text.
...
Starting a solo round calls `create_solo_session`. The database validates the
rules snapshot, supplies an opaque match ID, uint32 seed, and database start
time, and prevents a second active solo round. Submission uses the same
participant-scoped `/api/matches/results` route and Postgres
`submit_match_result` validator as multiplayer. The first valid completion is
immutable and updates the exact saved-mode row once.
...
- Menus and results can scroll; active play clips page scrolling while keeping
  the board, score, timer, current word, and accepted-word strip visible.
- The board supports Pointer Events, pointer cancellation, lost capture,
  backgrounding, rotation, and Visual Viewport/ResizeObserver remeasurement.
...
16. Complete solo and confirm exact-mode statistics update once.
17. Click Play Again and confirm a new seed.
18. Create a 3×3 private square.
19. Create an 8×8 private square.
20. Create a 3×8 and an 8×3 rectangle.
21. Compare 4×4 and 8×8 diamonds.
22. Compare 4×4 and 8×8 crosses.
23. Paint a custom mask by mouse/touch.
24. Edit the mask by keyboard.
25. Reject fewer than nine or disconnected custom cells.
26. Verify preview gaps exactly match the active-game gaps.
27. Join a private lobby from a second persistent account.
28. Start and compare exact preview boards on both devices.
29. Approve a unanimous reroll and compare the new boards.
30. Decline a reroll and confirm the original board starts on time.
31. Complete private results and compare placements/words/points.
32. Create a private rematch and accept its invitation.
33. Confirm copied rules/capacity and new seed/room code.
34. Queue two ranked accounts and leave a third waiting.
35. Confirm the eight-second ranked preview.
36. Complete ranked and compare opposite Elo deltas.
37. Request/accept a ranked rematch within 30 seconds.
38. Decline and separately expire a ranked rematch proposal.
39. Verify active-match guards reject duplicate rematches.
40. Test active gameplay at 320, 375, and 430 pixels.
41. Test phone portrait, landscape, safe areas, and rotation mid-drag.
42. Confirm valid/duplicate/invalid visual and spoken feedback.
43. Confirm accepted words use required result ordering.
44. Use the logo during active play and confirm the leave warning.
45. Install the PWA and inspect Cache Storage.
46. Confirm no auth, API, profile, match, or Realtime response is cached.
47. Go offline and confirm only the offline screen/guide are offered.
48. Inspect public RPC responses for UUID, email, token, and private data.
49. Refresh during queue, preview, game, result wait, and rematch proposal.
50. Confirm realtime reconnect and polling recovery without duplicate writes.
